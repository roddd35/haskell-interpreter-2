...
import Tokenizer (isNumeral, isSymbol, isBool, reservedWords, isAlphanumeric, isInitial, isReservedWord, isIdentifier, isValidIdentifier)
...
analyze tree = case tree of
  ...
  Leaf token
    | isValidIdentifier token -> IdS token
    | isNumeral token ->
  ...
  Pair first rest -> case first of
    ...
    Leaf ">"      -> GreaterTS (analyzePos 1) (analyzePos 2)
    Leaf "<"      -> LowerTS (analyzePos 1) (analyzePos 2)
    Leaf "=="     -> EqToS (analyzePos 1) (analyzePos 2)
    Leaf "!="     -> DiffOfS (analyzePos 1) (analyzePos 2)
    Leaf ">="     -> GreaterEqS (analyzePos 1) (analyzePos 2)
    Leaf "<="     -> LowerEqS (analyzePos 1) (analyzePos 2)
