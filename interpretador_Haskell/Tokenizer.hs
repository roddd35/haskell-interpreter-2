...
reservedWords = ["let", "call", "if", "cons", "head", "tail", "lambda", "letrec", "true", "false"]
  
isAlphanumeric :: Char -> Bool
isAlphanumeric c = isNumeralChar c || c `elem` ['a'..'z'] || c `elem` ['A'..'Z'] 

isInitial :: Char -> Bool
isInitial c = c `elem` ['a'..'z'] || c `elem` ['A'..'Z']

isReservedWord :: String -> Bool
isReservedWord str = str `elem` reservedWords

isIdentifier :: String -> Bool
isIdentifier [] = False 
isIdentifier (c:cs) = isInitial c && (null cs || all isAlphanumeric cs)

isValidIdentifier :: String -> Bool
isValidIdentifier str = isIdentifier str && not (isReservedWord str)